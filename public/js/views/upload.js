// Upload view: drag-and-drop MP3s into a new compilation. Same logic as the
// pre-SPA upload.html, repackaged as a view module.
...
const STATUS_LABEL = { pending: 'en attente', uploading: 'envoi…', done: 'ok', error: 'erreur', dup: 'doublon' };
...
export async function mount(el) {
  ...
  el.innerHTML = `
    <div class="shell-narrow">
      ...
      <div style="display:flex;gap:12px;justify-content:flex-end;margin-top:32px;">
        ...
        <button id="publish" class="btn-accent" disabled>Publier la compilation</button>
      </div>
    </div>
  `;
  ...
  function renderQueue() {
    ...
    items.forEach((it, i) => {
      ...
      li.innerHTML = `
        ...
        <div class="progress"><span style="width:${(it.progress * 100).toFixed(0)}%"></span></div>
        <div class="status ${it.status === 'done' ? 'ok' : it.status === 'error' ? 'err' : it.status === 'dup' ? 'dup' : ''}">${STATUS_LABEL[it.status] || it.status}</div>
      `;
  ...
  function updatePublishState() {
    el.querySelector('#publish').disabled = items.length === 0 || !el.querySelector('#title').value.trim();
  }
  el.querySelector('#title').addEventListener('input', updatePublishState);

  el.querySelector('#reset').addEventListener('click', () => {
    items.length = 0;
    coverFile = null;
    el.querySelector('#coverPreview').style.display = 'none';
  ...
  el.querySelector('#publish').addEventListener('click', async () => {
    ...
    try {
      ...
      if (coverFile) {
        await uploadCover({ file: coverFile, compilationId: compRef.id });
      }
      ...
      const tasks = items.map((it, idx) => async () => {
        ...
        try {
          ...
          it.progress = 1;
          it.status = result.dedupHit ? 'dup' : 'done';
          renderQueue();
          return result;
        } catch (err) {
          it.status = 'error';
      ...
      if (failures.length === 0) {
        ...
        navigate(`/c/${compRef.id}`);
      } else {
        throw new Error(`${failures.length} morceau(x) n'ont pas pu être envoyés. Corrige et republie.`);
      }
    } catch (err) {
      errEl.textContent = err.message || String(err);
      errEl.hidden = false;
      btn.disabled = false;
      btn.textContent = 'Publier la compilation';
    }
  });
}
